...
const isValidCheckValue = (value, props) => value === props.checkedValue ||
...
const getComponentStyle = (props, state) => {
    const { focused } = state;
    const { disabled, focusedStyle, disabledStyle, readOnly, readOnlyStyle, style, } = props;
    const styles = [style || {}];
    if (focused) {
...
const getComputedIconStyle = config => {
    const { focused, disabled, focusedIconStyle, disabledIconStyle, iconStyle, } = config;
...
const renderCheckedIcon = (props, iconProps) => {
    ...
    if (checkedIconSrc) {
        return React.createElement("img", { ...iconProps, src: checkedIconSrc });
    }
...
const renderUncheckedIcon = (props, iconProps) => {
    ...
    if (uncheckedIconSrc) {
        return React.createElement("img", { ...iconProps, src: uncheckedIconSrc });
    }
...
const renderIndeterminateIcon = (props, iconProps) => {
    ...
    if (indeterminateIconSrc) {
        return React.createElement("img", { ...iconProps, src: indeterminateIconSrc });
    }
...
const renderHiddenInput = props => {
    ...
    if (withHiddenInput) {
        return React.createElement("input", { type: "hidden", name: name, value: checked });
    }
    return null;
};
...
class InovuaCheckBox extends Component {
    ...
    render() {
        ...
        const { children, className, style, tabIndex, iconCheckOnly, browserNative, focusable, } = (this.p = this.getProps(props, state));
        const eventHandlers = {
        ...
        return browserNative ? (this.renderBrowserNative(domProps)) : (React.createElement("div", { ...domProps },
            this.renderCheckbox(this.p, iconCheckOnly && { onClick: this.handleClick }),
            children && (React.createElement("div", { className: `${props.rootClassName}__inner-content-wrapper` }, children))));
    }
    renderBrowserNative(domProps) {
        const { children, iconCheckOnly } = this.props;
        const Factory = iconCheckOnly ? 'div' : 'label';
        return (React.createElement(Factory, { ...domProps, onClick: this.props.onClick, onChange: stopPropagation },
            this.renderBrowserNativeInput(),
            children));
    }
    renderBrowserNativeInput() {
        const { tabIndex, rootClassName, disabled, checked, renderNativeBrowserInput, withHiddenInput, name, } = this.p;
        const className = join(this.p.className, `${rootClassName}--browser-native`);
        const inputProps = {
            disabled,
            className,
            type: 'checkbox',
    ...
    renderCheckbox(config, eventHandlers) {
        ...
        return (React.createElement("div", { className: `${config.rootClassName}__icon-wrapper`, ...domProps, ...eventHandlers },
            input,
            icon));
    }
