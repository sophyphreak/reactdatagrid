...
class InovuaInertialScroller extends Component {
    ...
    render() {
        const { props } = this;
        const className = join(props.rootClassName, props.className, `${props.rootClassName}--theme-${props.theme}`);
        let events;
        ...
        return (React.createElement("div", { ...cleanProps(props, InovuaInertialScroller.propTypes), className: className, ref: this.setRootRef, ...events },
            React.createElement("div", { ref: this.setViewRef, className: `${props.rootClassName}__view` }, props.children)));
    }
    tap(event) {
        this.pressed = true;
