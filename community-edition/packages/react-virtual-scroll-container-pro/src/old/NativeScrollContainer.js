...
const onScroll = function ({ target }) {
    ...
    const { scrollTop: prevScrollTop, scrollLeft: prevScrollLeft, } = prevScrollPos;
    if (startStop) {
    ...
    if (scrollTopChange) {
        ...
        if (this.props.onContainerScrollVerticalMax &&
    ...
    if (scrollLeftChange) {
        ...
        if (this.props.onContainerScrollHorizontalMax &&
...
const InovuaNativeScrollContainer = ScrollerFactory('InovuaNativeScrollContainer', {
    init(props) {
        this.refScroller = s => {
            this.scroller = s;
        };
    ...
    sync() {
        ...
        if (scrollbars.vertical !== this.oldScrollbars.vertical ||
    ...
    renderScroller({ content, spacer, scrollerStyle, props }) {
        ...
        const result = typeof renderScroller === 'function' ? (renderScroller(domProps)) : (React.createElement("div", { ...domProps }));
        return result;
    },
});
