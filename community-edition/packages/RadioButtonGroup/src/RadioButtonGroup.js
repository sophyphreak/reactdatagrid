...
const getClassNames = (props, state) => {
    ...
    return join(CLASS_NAME, props.className, `${CLASS_NAME}--theme-${theme}`, `${CLASS_NAME}--orientation-${orientation}`, stretch && `${CLASS_NAME}--stretch`, horizontal && `${CLASS_NAME}--orientation-horizontal`, disabled && `${CLASS_NAME}--disabled`, rtl ? `${CLASS_NAME}--rtl` : `${CLASS_NAME}--ltr`, state.focused && `${CLASS_NAME}--focused`);
};
...
const getRadioBasedChildren = ({ radioOptions, renderItem, currentCheckedValue, onChange, disabled, readOnly, rtl, theme, }) => {
...
class InovuaRadioButtonGroup extends Component {
    ...
    getProps(props, state) {
        ...
        let cleanChildren;
        const currentCheckedValue = (this.currentCheckedValue = getCurrentCheckedValue(props, state));
    ...
    renderHiddenInput() {
        const { tabIndex, disabled, checked, withHiddenInput, name, shouldSubmit, } = this.p;
        const inputProps = {
            type: 'checkbox',
        ...
        return React.createElement("input", { ...inputProps, type: "hidden" });
    }
        ...
        switch (ev.key) {
            ...
            case KEYS.RIGHT_ARROW:
                ...
                if (focusedIndex < children.length - 1) {
                    const nextValue = children[focusedIndex + 1].props['data-radio-value'];
            ...
            case KEYS.LEFT_ARROW:
                ev.preventDefault();
                if (focusedIndex > 0) {
                    const prevValue = children[focusedIndex - 1].props['data-radio-value'];
    ...
    render() {
        const { children, className, name, currentCheckedValue, submitFriendly, enableKeyboardNavigation, } = (this.p = this.getProps());
        const cleanedProps = cleanProps(this.props, InovuaRadioButtonGroup.propTypes);
        ...
        return (React.createElement("div", { ...computedRBGDOMProps },
            children,
            submitFriendly &&
...
InovuaRadioButtonGroup.defaultProps = {
    ...
    submitFriendly: true,
    renderItem: props => React.createElement(WrappedRadioInput, { ...props }),
...
const WrappedRadioInput = props => {
    ...
    return (React.createElement(RadioButton, { iconSize: 16, inlineBlock: false, ...props, checkedValue: true, uncheckedValue: false, onChange: onChange }));
};
const WrappedCheckBox = props => {
    ...
    return (React.createElement(CheckBox, { iconSize: 16, ...props, checkedValue: true, uncheckedValue: false, onChange: onChange }));
};
