...
const GROUP_CELLS_SELECTOR = '.InovuaReactDataGrid__cell--group-cell:not(.InovuaReactDataGrid__cell--hidden)';
...
describe('DataGrid.groupBy Cell', () => {
    it('should render group cells of correct width, when grid has no scroll', done => {
        const gridInstance = render(columns, { [{ name: 'firstName', width: 100 },
            ...
            ]:  }, idProperty = "id", groupBy = { ['country']:  }, dataSource = { [{
                firstName: 'john',
    ...
    it('should render group cells of correct width, when grid has scroll & hideGroupByColumns is false', done => {
        const gridInstance = render(columns, { [{ name: 'firstName', width: 100 },
            ...
            ]:  }, hideGroupByColumns = { false:  }, idProperty = "id", groupBy = { ['country']:  }, renderGroupTitle = {}({ value, namePath }));
...
describe('DataGrid.groupBy Cell - with locked cells', () => {
    it('should render locked group cells of correct width, when grid has no scroll', done => {
        const gridInstance = render(columns, { [{ name: 'firstName', width: 100, locked: true },
        ...
        setTimeout(() => {
            ...
            expect(countryGroupCells[1].offsetWidth).to.equal(300);
            // get the first row
            const row1 = findDOMNode(gridInstance).querySelector('.InovuaReactDataGrid__row');
    ...
    it('should render locked group cells of correct width, when grid has scroll & hideGroupByColumns is false', done => {
        const gridInstance = render(columns, { [{ name: 'country', width: 3000 },
                { name: 'firstName', width: 100, locked: true },
                { name: 'lastName', width: 200, locked: true },
                { name: 'email', width: 300 },
                { name: 'x', width: 400 },
            ]:  }, hideGroupByColumns = { false:  }, groupNestingSize = { 40:  }, idProperty = "id", groupBy = { ['country']:  }, dataSource = { [{
                firstName: 'john',
                email: 'john@gmail.com',
                id: 1,
                country: 'uk',
