...
const dragSetup = (gridInstance, { index: colIndex, diff }, dropProps) => {
    ...
    return {
        ...
        drag: () => {
            ...
            const currentTarget = colHeaderNode.querySelector('.InovuaReactDataGrid__column-resize-handle');
            const shareSpace = dropProps && dropProps.shareSpace !== undefined
            ...
            body.columnLayout.onResizeDrop(assign({}, {
                index: colIndex,
                offset: 0,
                diff,
...
describe('DataGrid Column resize (no groups)', () => {
    it('should correctly resize a flex column with keepFlex', done => {
        ...
        const gridInstance = render(React.createElement(DataGrid, { onColumnResize: onColumnResize, dataSource: dataSource, idProperty: "id", style: { width: 900 }, columnDefaultWidth: 100, columns: columns }));
        setTimeout(() => {
            const { headers, drag } = dragSetup(gridInstance, { index: 2, diff: -98 }, { shareSpace: true, size: null });
            ...
            const g = gridInstance.rerender(React.createElement(DataGrid, { dataSource: dataSource, idProperty: "id", style: { width: 1000 }, columnDefaultWidth: 100, columns: columns }));
    ...
    it('should correctly resize a flex column without keepFlex', done => {
        ...
        const gridInstance = render(React.createElement(DataGrid, { onColumnResize: onColumnResize, dataSource: dataSource, idProperty: "id", style: { width: 900 }, columnDefaultWidth: 100, columns: columns }));
        setTimeout(() => {
            const { headers, drag } = dragSetup(gridInstance, { index: 2, diff: -98 }, { shareSpace: false, size: null });
            expect(headers.map(h => h.style.width)).to.eql([
                '100px',
                '100px',
                '598px',
    ...
    it('should correctly resize a flex column', done => {
        ...
        const dataSource = [{ firstName: 'john', email: 'john@gmail.com', id: 1 }];
        const gridInstance = render(React.createElement(DataGrid, { dataSource: dataSource, idProperty: "id", style: { width: 1000 }, columnDefaultWidth: 100, columns: columns }));
        setTimeout(() => {
            ...
            gridInstance.rerender(React.createElement(DataGrid, { dataSource: dataSource, idProperty: "id", style: { width: 900 }, columnDefaultWidth: 100, columns: columns }));
    ...
    it('should correctly resize a nonflex column, with a flex sibling and shareSpaceOnResize', done => {
        ...
        const dataSource = [{ firstName: 'john', email: 'john@gmail.com', id: 1 }];
        const gridInstance = render(React.createElement(DataGrid, { dataSource: dataSource, idProperty: "id", style: { width: 900 }, columnDefaultWidth: 100, columns: columns, shareSpaceOnResize: true }));
        setTimeout(() => {
            const { headers, drag } = dragSetup(gridInstance, {
                index: 1,
                diff: -30,
            ...
            gridInstance.rerender(React.createElement(DataGrid, { dataSource: dataSource, idProperty: "id", style: { width: 1000 }, columnDefaultWidth: 100, shareSpaceOnResize: true, columns: columns }));
    ...
    it('should correctly resize columns in a simple case - all non-flex columns, shareSpaceOnResize=false', done => {
        const gridInstance = render(React.createElement(DataGrid, { dataSource: [{ firstName: 'john', email: 'john@gmail.com', id: 1 }], idProperty: "id", columnDefaultWidth: 100, columns: [
                { name: 'firstName' },
                { name: 'lastName' },
                { name: 'age' },
                { name: 'email' },
    ...
    it('should correctly resize columns with shareSpaceOnResize', done => {
        const gridInstance = render(React.createElement(DataGrid, { shareSpaceOnResize: true, dataSource: [{ firstName: 'john', email: 'john@gmail.com', id: 1 }], idProperty: "id", columnDefaultWidth: 200, columns: [
                { name: 'firstName' },
                { name: 'lastName' },
                { name: 'age' },
                { name: 'email' },
