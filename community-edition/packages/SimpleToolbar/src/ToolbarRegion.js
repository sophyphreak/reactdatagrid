...
export default class InovuaToolbarRegion extends React.Component {
    render() {
        ...
        const className = join(CLASS_NAME, props.align && `${CLASS_NAME}--align-${props.align}`, props.theme && `${CLASS_NAME}--theme-${props.theme}`, textAlign && `${CLASS_NAME}--text-align-${textAlign}`, props.orientation && `${CLASS_NAME}--orientation-${props.orientation}`);
        return (React.createElement(Item, { ...cleanProps(props, InovuaToolbarRegion.propTypes), className: className, justifyContent: justifyContent, flexFlow: flexFlow }));
    }
}
