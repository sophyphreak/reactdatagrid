...
export const renderNavBar = function (config, navBarProps) {
    ...
    return (React.createElement(NavBar, { key: "multi_month_nav_bar", style: marginStyle, ...navBarProps }));
};
export default class MultiMonthView extends Component {
    ...
    render() {
        ...
        return (React.createElement(Flex, { key: "multi_month_view", column: true, inline: true, alignItems: "stretch", wrap: false, ...flexProps, className: className }, children));
    }
    renderRow(rowIndex) {
        const props = this.p;
        ...
        return (React.createElement(Flex, { key: `row_index_${rowIndex}`, inline: true, row: true, wrap: false, children: children }));
    }
    renderView(viewProps, index, size, keys) {
        ...
        return (React.createElement(MonthView, { ref: view => {
                this.views[index] = view;
            }, constrainViewDate: false, ...viewProps, className: null, index: index, key: keys, footer: false, constrainActiveInView: false, navigate: this.onMonthNavigate.bind(this, index), hoverRange: this.state.hoverRange, onHoverRangeChange: this.setHoverRange, activeDate: props.activeDate, onActiveDateChange: this.onActiveDateChange, onViewDateChange: this.onAdjustViewDateChange, date: props.date, defaultDate: null, onChange: this.onChange, range: range, defaultRange: null, onRangeChange: this.onRangeChange, viewMoment: viewMoment, insideMultiView: true, daysInView: props.daysInView[index], showDaysBeforeMonth: index == 0, showDaysAfterMonth: index == size - 1, select: this.select, renderNavBar: this.props.navigation &&
    ...
    renderHiddenNav(props) {
        return React.createElement(InlineBlock, { ...props, style: { visibility: 'hidden' } });
    }
    isInRange(moment) {
...
MultiMonthView.propTypes = {
    ...
    viewMoments: PropTypes.arrayOf(PropTypes.oneOfType([PropTypes.object, PropTypes.string, PropTypes.number])),
