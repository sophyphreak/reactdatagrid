...
export default {
    ...
    notifySelection(selected, data, unselected) {
        ...
        if (!this.isUnselectedControlled()) {
            const unselectedCount = this.getUnselectedCount(undefined, undefined, unselected);
            handled = true;
            this.update({
                unselected,
                unselectedCount,
    ...
    handleSingleSelection(data, event) {
        ...
        if (!this.props.toggleRowSelectOnClick &&
            rowSelected &&
            event &&
            !ctrlKey) {
    ...
    handleSelection(rowProps, event) {
        ...
        if (event.metaKey ||
            ...
            const unselect = this.handleMultiSelectionRowToggle(props.data[index], event);
            if (unselect) {
                this.selIndex++;
