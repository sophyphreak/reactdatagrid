...
import getTransformedStringValues, { getDecimalDelimiterPosition, } from './utils/get-transformed-string-values';
...
const getArrowClasses = (spinDirection, props) => {
    const upArrowClass = join(`${props.rootClassName}__spinner-arrow`, `${props.rootClassName}__spinner-arrow--up`, spinDirection === 1 && `${props.rootClassName}__spinner-arrow--active`);
    const downArrowClass = join(`${props.rootClassName}__spinner-arrow`, `${props.rootClassName}__spinner-arrow--down`, spinDirection === -1 && `${props.rootClassName}__spinner-arrow--active`);
    return { upArrowClass, downArrowClass };
};
const getClearButtonClassNames = props => {
    const clearButtonClassName = join(`${props.rootClassName}__clear-button`, `${props.rootClassName}__clear-button--${props.rtl ? 'rtl' : 'ltr'}`, props.clearButtonClassName);
...
const getSuffix = props => {
    ...
    if (format === 'currency' && currencyPosition === 'end') {
        ...
        return getCurrencyForCountryCode(locale || (globalObject.navigator ? globalObject.navigator.language : ''));
    }
...
class InovuaNumericInput extends Component {
    constructor(props) {
        ...
        if (props.triggerChangeOnSpinDelay > 0) {
            this.setValueOnSpin = debounce(this.setValueOnSpin, props.triggerChangeOnSpinDelay);
        }
        this.state = {
            value,
            focused: false,
    ...
    render() {
        ...
        const { formattedValue, enableSpinnerTools, className, style, enableClearButton, wrapperProps, } = (this.p = this.getProps(props, state));
        let min = props.mim;
        ...
        const inputClassName = join(`${props.rootClassName}__input`, inputProps.className);
        const input = (React.createElement("input", { size: Math.max(1, props.size || 0), placeholder: this.props.placeholder, ...inputProps, ref: this.setInputRef, type: "text", key: "input", className: inputClassName, disabled: props.disabled, value: currentValue, onWheel: this.handleWheel, onKeyDown: this.handleKeyDown, onKeyUp: this.handleKeyUp, onChange: this.handleChange, min: min }));
        ...
        const propsToWrapper = cleanProps(wrapperProps, InovuaNumericInput.propTypes);
        return (React.createElement("div", { ...propsToWrapper, className: join(className, wrapperProps ? wrapperProps.className : null), style: wrapperProps && wrapperProps.style
                ? { ...style, ...wrapperProps.style }
                : style, onBlur: this.handleBlur, onFocus: this.handleFocus }, inputDisplay));
    }
    renderClearButtonWrapper() {
        ...
        const hasValue = state.intermediateValue != null ? true : value !== '' && value != null;
        const clearButtonWrapperClassName = join(`${props.rootClassName}__clear-button-wrapper`, (!hasValue || !enableClearButton) &&
            `${props.rootClassName}__clear-button-wrapper--hidden`);
        return (React.createElement("div", { key: "clearButton", className: clearButtonWrapperClassName }, this.renderClearButton({
            clearButtonColor,
            clearButtonStyle,
            clearButtonSize,
    ...
    renderClearButton(config) {
        const { clearButtonColor, clearButtonStyle, clearButtonClassName, clearButtonSize, } = config;
        const svgProps = {};
        ...
        return (React.createElement("button", { key: "clearButton", type: "button", onClick: this.handleClearButtonClick, onMouseDown: preventDefault, className: clearButtonClassName, style: { ...clearButtonStyle }, tabIndex: tabIndex }, this.renderClearIcon(svgProps)));
    }
    renderClearIcon = svgProps => {
        let result;
        if (this.props.renderClearIcon &&
        ...
        if (result === undefined) {
            result = (React.createElement("svg", { style: { ...svgProps }, viewBox: "0 0 10 10" },
    ...
    renderSpinnerToolsWrapper() {
        ...
        const { arrowColor, arrowStyle, arrowUpStyle, arrowDownStyle, spinDirection, arrowSize, } = this.p;
        const spinnerToolsWrapperClassName = enableSpinnerTools
            ? `${props.rootClassName}__spinner-wrapper ${props.rootClassName}__spinner-wrapper--${rtl ? 'rtl' : 'ltr'}`
            : `${props.rootClassName}__spinner-wrapper-hidden ${props.rootClassName}__spinner-wrapper-hidden--${rtl ? 'rtl' : 'ltr'}`;
        return (React.createElement("div", { key: "spinnerTool", className: spinnerToolsWrapperClassName }, this.renderSpinnerTools({
            arrowColor,
            arrowStyle,
            arrowUpStyle,
            arrowDownStyle,
    ...
    renderSpinnerTools(config) {
        const { arrowColor, arrowStyle, arrowUpStyle, arrowDownStyle, upArrowClass, downArrowClass, arrowSize, } = config;
        const svgProps = {};
        if (arrowColor) {
        ...
        return [
            React.createElement("span", { key: "up", ...arrowEvents.upEvents, className: upArrowClass, style: { ...arrowStyle, ...arrowUpStyle } }, this.renderArrowUpIcon(svgProps)),
            React.createElement("span", { key: "down", ...arrowEvents.downEvents, className: downArrowClass, style: { ...arrowStyle, ...arrowDownStyle } }, this.renderArrowDownIcon(svgProps)),
        ];
    }
    renderArrowUpIcon = svgProps => {
        const { height } = svgProps;
        let result;
        if (this.props.renderArrowUpIcon &&
            typeof this.props.renderArrowUpIcon === 'function') {
            result = this.props.renderArrowUpIcon({ ...svgProps });
        }
        if (result === undefined) {
            result = (React.createElement("svg", { style: { ...svgProps, height: height / 2 }, viewBox: "0 0 10 5" },
                React.createElement("path", { fillRule: "evenodd", d: "M5.262.262l4.106 4.106c.144.144.144.379 0 .524-.07.069-.164.108-.262.108H.894c-.204 0-.37-.166-.37-.37 0-.099.039-.193.108-.262L4.738.262c.145-.145.38-.145.524 0z" })));
        }
        return result;
    };
    renderArrowDownIcon = svgProps => {
        const { height } = svgProps;
        let result;
        if (this.props.renderArrowDownIcon &&
            typeof this.props.renderArrowDownIcon === 'function') {
            result = this.props.renderArrowDownIcon({ ...svgProps });
        }
        if (result === undefined) {
            result = (React.createElement("svg", { style: { ...svgProps, height: height / 2 }, viewBox: "0 0 10 5" },
                React.createElement("path", { fillRule: "evenodd", d: "M4.738 4.738L.632.632C.488.488.488.253.632.108.702.04.796 0 .894 0h8.212c.204 0 .37.166.37.37 0 .099-.039.193-.108.262L5.262 4.738c-.145.145-.38.145-.524 0z" })));
        }
        return result;
    };
    getArrowEvents() {
    ...
    getTransformedStringValues(value) {
        const { suffix, prefix, locale, precision, isControlledPrecision, minValue, maxValue, } = this.p;
        return getTransformedStringValues(value, {
            locale,
    ...
    handleBlur(event) {
        ...
        if (this.state.intermediateValue != null) {
            let value = clamp(parseFloat(this.state.intermediateValue), this.props.minValue, this.props.maxValue);
    ...
    setValue(value, { silent } = {}) {
        ...
        const isSameRepresentation = parsedValue === null || `${value}` === parsedValue.toString();
    ...
    handleBackspaceKeyDown(event) {
        ...
        if (isControlledPrecision) {
            ...
            const decimalPosition = getDecimalDelimiterPosition(currentValue, decimalDelimiter);
    ...
    handleSelectionOverDecimalDelimiter(event, currentSelection) {
        const { decimalDelimiter, currentValue, isControlledPrecision, prefix, suffix, } = this.p;
        ...
        let selectionEndsAtEndOfNumber = currentSelection.end === currentValue.length;
        if (suffix) {
    ...
    getStepValue(props, direction, config) {
        ...
        return clamp(Math.round(theValue * 1000) / 1000, this.props.minValue, this.props.maxValue);
    }
    stepTo(direction, config) {
        ...
        if (step != null) {
            const stepFn = typeof props.stepFn === 'function' ? props.stepFn : this.getStepValue;
    ...
    handleWheel(event) {
        const props = this.props;
        if (((props.requireFocusOnStep && this.isFocused()) ||
            !props.requireFocusOnStep) &&
            props.stepOnWheel &&
            props.step) {
            event.preventDefault();
            const nativeEvent = event.nativeEvent;
            let y = nativeEvent.wheelDeltaY ||
    ...
    getProps(props, state) {
        ...
        const decimalDelimiterPosition = getDecimalDelimiterPosition(currentValue, decimalDelimiter);
        const className = join(props.rootClassName, props.theme && `${props.rootClassName}--theme-${props.theme}`, `${props.rootClassName}--${props.rtl ? 'rtl' : 'ltr'}`, props.disabled && `${props.rootClassName}--disabled`, state.focused && `${props.rootClassName}--focused`, props.enableSpinnerTools &&
            `${props.rootClassName}--enable-spinner-tools`, props.enableClearButton && `${props.rootClassName}--enable-clear-button`, props.className);
