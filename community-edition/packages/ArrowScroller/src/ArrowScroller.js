...
class InovuaArrowScroller extends Component {
    ...
    componentDidMount() {
        raf(() => {
            ...
            this.scrollInfo.scrollerSize = {
                start: this.props.scroller === (true || 'auto') ? this.start[name] : null,
    ...
    render() {
        const { props } = this;
        const { useTransformOnScroll, nativeScroll, vertical, rootClassName, rtl, theme, } = props;
        ...
        const resizer = (React.createElement(NotifyResize, { key: "resizer", notifyOnMount: true, onResize: this.handleResize, notifyResizeDelay: props.notifyResizeDelay }));
        const children = [...React.Children.toArray(props.children), resizer];
        const content = (React.createElement(Flex, { key: "content", wrap: false, ...props.childProps, className: innerWrapperClassName, ref: this.strip, children: children, style: nativeScroll ? null : moveStyle }));
        let finalChildren = [
            resizer,
            content,
        ...
        if (nativeScroll) {
            ...
            finalChildren = (React.createElement(ScrollContainer, { applyCSSContainOnScroll: false, style: style, scrollerStyle: scrollerStyle, emptyScrollOffset: 20, getScrollerNodeClientSize: this.props.rtl ? this.getScrollerNodeClientSize : null, className: `${this.props.rootClassName}__scroll-container`, shouldAllowScrollbars: NO_SCROLLBARS, dragToScroll: false, nativeScroll: scrollContainerProps
                    ? scrollContainerProps.nativeScroll !== undefined
                        ? scrollContainerProps.nativeScroll
                        : false
                    : false, viewStyle: viewStyle, ref: this.refScrollContainer, scrollerResizerProps: scrollerResizerProps, onDidMount: this.onScrollContainerDidMount, onWillUnmount: this.onScrollContainerWillUnmount, onResize: scrollContainerProps && scrollContainerProps.onResize
                    ? callAll(scrollContainerProps.onResize, this.rafUpdateScrollInfo)
                    : this.rafUpdateScrollInfo, children: prevChildren, onContainerScroll: this.onContainerScroll, theme: props.theme }));
            if (this.props.rtl) {
    ...
    renderScroller(direction) {
        ...
        const arrowName = vertical
            ...
            : direction == (this.props.rtl ? 1 : -1)
                ...
                : 'right';
        const scrollInfo = this.scrollInfo;
        const disabled = direction == -1 ? !scrollInfo.hasStartScroll : !scrollInfo.hasEndScroll;
    ...
    renderArrowIcon(name) {
        const { props } = this;
        return (React.createElement(Arrow, { name: name, size: props.arrowSize, height: props.arrowHeight, width: props.arrowWidth, className: `${this.props.rootClassName}__arrow-icon` }));
    }
    getOffsetSizeName() {
