...
class InovuaToolbar extends Component {
    ...
    render() {
        ...
        return props.overflowStrategy === 'scroll' ? (React.createElement(InovuaArrowScroller, { ...rootProps, ...this.getScrollerProps() })) : (React.createElement(DropdownOverflow, { ...rootProps, ...this.getDropdownOverflowProps() }));
    }
    scrollIntoView(node) {
    ...
    getClassName() {
        const { props } = this;
        return join(this.props.className, props.rootClassName, props.theme && `${props.rootClassName}--theme-${props.theme}`, props.changeButtonStyles && `${props.rootClassName}--change-button-styles`);
    }
    getScrollerProps() {
        const { props } = this;
        const result = {
            ...
            className: join(this.getClassName(), `${props.rootClassName}--arrowScroller`),
        };
        if (this.props.rtl) {
...
const Separator = props => {
    return (React.createElement("div", { ...props, className: join(props.className, 'inovua-react-toolkit-toolbar__separator') }));
};
