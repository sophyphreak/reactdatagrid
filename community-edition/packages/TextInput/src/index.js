...
const getClearButtonClassNames = props => {
    const clearButtonClassName = join(`${props.rootClassName}__clear-button`, props.clearButtonClassName);
...
class TextInput extends Component {
    ...
    render() {
        ...
        const className = join(props.rootClassName, props.className, props.rtl ? `${props.rootClassName}--rtl` : `${props.rootClassName}--ltr`, props.theme && `${props.rootClassName}--theme-${props.theme}`, enableClearButton && `${props.rootClassName}--enable-clear-button`, state.focused && `${props.rootClassName}--focused`, fieldProps.disabled && `${props.rootClassName}--disabled`);
        const input = React.createElement(Field, { ...fieldProps });
        ...
        return (React.createElement("div", { ...domProps, ...wrapperDomProps, className: className, style: style, onBlur: this.onBlur, onClick: this.onClick, onFocus: this.onFocus },
            input,
    ...
    renderClearButtonWrapper(fieldProps) {
        const { props, state } = this;
        const { clearButtonColor, clearButtonStyle, clearButtonSize, enableClearButton, rootClassName, } = props;
        const value = isControlled(props) ? props.value : state.value;
        const emptyValue = value == '' || value == null;
        const showButton = enableClearButton &&
            !emptyValue &&
            !fieldProps.disabled &&
            !fieldProps.readOnly;
        return (React.createElement("div", { key: "clearButton", className: join(`${rootClassName}__clear-button-wrapper`, !showButton && `${rootClassName}__clear-button-wrapper--hidden`) }, this.renderClearButton({
            clearButtonColor,
            clearButtonStyle,
            clearButtonSize,
