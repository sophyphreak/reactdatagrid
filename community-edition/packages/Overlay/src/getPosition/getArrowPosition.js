...
function getArrowPosition({ overlayRegion, targetRegion, position, arrowSize, }) {
    ...
    const left = clamp(getLeftPosition(overlayRegion, targetRegion), arrowSize / 2, overlayRegion.width - arrowSize / 2);
    const top = clamp(getTopPosition(overlayRegion, targetRegion), arrowSize / 2, overlayRegion.height - arrowSize / 2);
