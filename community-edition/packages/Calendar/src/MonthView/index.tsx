...
const renderFooter = (props, buttonHandlers) => {
  ...
  const childFooter = React.Children.toArray(props.children).filter(
  ...
  if (childFooterProps) {
    ...
    Object.keys(footerFnProps).forEach(key => {
      if (childFooter.props[key]) {
        footerFnProps[key] = joinFunctions(
          footerFnProps[key],
          childFooter.props[key]
        );
      }
    });
  }
  ...
  const footerProps = assignDefined({}, footerFnProps, {
    ...
    okButton:
      props.okButton === undefined && !props.insideField
        ? false
        : props.okButton,
    ...
    cancelButton:
      props.cancelButton === undefined && !props.insideField
        ? false
        : props.cancelButton,
...
export default class MonthView extends Component {
  ...
  prepareClassName(props) {
    return join(
      ...
      props.size === undefined ? `${props.rootClassName}-relative` : null
    );
  }
  ...
  preparePrevNextClassName(timestamp, props) {
    ...
    return join(
      timestamp == TODAY && `${rootClassName}-day--today`,
      props.highlightToday &&
        timestamp == TODAY &&
        `${rootClassName}-day--today-highlight`,
      before && `${rootClassName}-day--prev-month`,
      before && !props.showDaysBeforeMonth && `${rootClassName}-day--hidden`,
      after && `${rootClassName}-day--next-month`,
      after && !props.showDaysAfterMonth && `${rootClassName}-day--hidden`,
      thisMonth && `${rootClassName}-day--this-month`
    );
  }
  ...
  prepareWeekendClassName(dateMoment, { highlightWeekends }) {
    ...
    if (weekDay === 0 /* Sunday */ || weekDay === 6 /* Saturday */) {
      return join(
        `${rootClassName}-day--weekend`,
        highlightWeekends && `${rootClassName}-day--weekend-highlight`
      );
    }
  ...
  prepareDayProps(renderDayProps, props) {
    ...
    assign(result, minMaxProps, rangeProps, {
      children: (
        <div className={`${rootClassName}-day-text`}>{renderDayProps.day}</div>
      ),
      className: join([
  ...
  renderDay(renderProps) {
    ...
    if (props.range && props.highlightRangeOnMouseMove) {
      renderProps.onMouseEnter = this.handleDayMouseEnter.bind(
        this,
        renderProps
      );
    }
  ...
  render() {
    ...
    return (
      <BasicMonthView
        ...
        onMouseLeave={
          props.highlightRangeOnMouseMove ? this.handleViewMouseLeave : null
        }
      />
    );
  }
  ...
  renderNavBar(props) {
    ...
    const childNavBar = React.Children.toArray(props.children).filter(
