...
export default (cellProps, rowProps) => {
    const { groupProps, totalLockedStartWidth, totalLockedEndWidth, totalUnlockedWidth, totalComputedWidth, totalColumnCount, firstLockedEndIndex, firstUnlockedIndex, groupColumn, hasLockedEnd, hasLockedStart, } = rowProps;
    ...
    if ((groupProps && computedVisibleIndex < groupProps.depth) ||
    ...
    if (computedVisibleIndex === groupProps?.depth + 1 || expandColumn) {
        ...
        if (expandGroupTitle || expandColumn) {
            ...
            cellProps.computedColspan = Math.max(lockedStart
                ? firstUnlockedIndex - computedVisibleIndex
                : lockedEnd
                    ? totalColumnCount - computedVisibleIndex
                    : firstLockedEndIndex - computedVisibleIndex, 1);
        }
    ...
    if (lockedStart) {
        ...
        if (!cellProps.hidden) {
            ...
            cellProps.lastInSection = true;
            cellProps.computedColspan = Math.max(lockedEnd
                ? totalColumnCount - computedVisibleIndex
                : firstLockedEndIndex - computedVisibleIndex, 1);
        }
    }
