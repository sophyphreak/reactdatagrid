...
export default class Clock extends Component {
    ...
    render() {
        ...
        const width = size;
        const height = size;
        const className = join(props.className, props.rootClassName, `${props.rootClassName}--theme-${props.theme}`);
        let seconds;
        let minutes;
        let hours;
        ...
        return (React.createElement("div", { ...divProps, className: className, style: style },
            this.renderCenter(),
            ...
            this.props.size == 'auto' && (React.createElement(NotifyResize, { notifyOnMount: true, onResize: this.onResize }))));
    }
    renderCenter() {
        const props = this.props;
        const centerSize = props.centerSize || (props.bigTickHeight || props.tickHeight) * 3;
        return (React.createElement("div", { className: `${props.rootClassName}-center`, style: { width: centerSize, height: centerSize } }));
    }
    renderCenterOverlay() {
        const props = this.props;
        const centerOverlaySize = props.centerOverlaySize || props.handWidth * 4;
        return (React.createElement("div", { className: `${props.rootClassName}-overlay`, style: {
    ...
    renderHand(name, value) {
        ...
        const className = join(`${props.rootClassName}-hand`, `${props.rootClassName}-hand-${name}`);
        ...
        return React.createElement("div", { key: name, className: className, style: style });
    }
    renderTick(tick) {
        if (this.ignoreRender) {
            return null;
        }
        const { size, borderWidth, tickWidth, smallTickWidth, bigTickWidth, tickHeight, smallTickHeight, bigTickHeight, tickOffset, smallTickOffset, bigTickOffset, rootClassName, } = this.p;
        const small = !!(tick % 5);
        const sizeName = small ? 'small' : 'big';
        if (small && !this.props.showSmallTicks) {
            return false;
        }
        const className = join(`${rootClassName}-tick`, `${rootClassName}-tick--${sizeName}`);
        const offset = small
        ...
        return React.createElement("div", { key: tick, className: className, style: style });
    }
}
Clock.defaultProps = {
