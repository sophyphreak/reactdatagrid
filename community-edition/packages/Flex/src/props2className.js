...
export default props => {
    ...
    let className = join(props.className, props.alignItems ? `${PREFIX}--align-items-${props.alignItems}` : null, props.alignContent
        ? `${PREFIX}--align-content-${props.alignContent}`
        : null, props.justifyContent
        ? `${PREFIX}--justify-content-${props.justifyContent}`
        : null, props.wrap ? `${PREFIX}--wrap` : null, props.alignSelf ? `${PREFIX}--align-self-${props.alignSelf}` : null, row ? `${PREFIX}--row${reverse}` : null, column ? `${PREFIX}--column${reverse}` : null, 
