...
export default class InovuaDataGridList extends Component {
    ...
    setupActiveRowIndicatorHandle = (activeRowHandle) => {
    ...
    renderSizer = (props, scrollHeight) => {
        ...
        return (React.createElement("div", { key: "grid-sizer", "data-name": "sizer", ...props, style: sizerStyle }));
    };
    ...
    renderScroller = scrollerProps => {
        ...
        if (result === undefined) {
            result = React.createElement("div", { ...scrollerProps });
        }
        return result;
    };
    getRowAt = index => {
    ...
    onContainerScroll = (scrollPos, prevScrollPos = DEFAULT_SCROLL_POS) => {
    ...
    updateOnScrollLeft = (scrollLeft = this._scrollLeft, force = false, props = this.props) => {
        if (this.__willUnmount) {
            return;
        }
