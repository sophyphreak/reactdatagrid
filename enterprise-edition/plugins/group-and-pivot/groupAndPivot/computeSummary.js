...
export const computeSummary = (dataArray, config) => {
    ...
    if ((config.groupSummaryReducer || config.groupColumnSummaryReducers) &&
        Array.isArray(dataArray)) {
        const columnsWithSummaries = Object.keys(config.groupColumnSummaryReducers || {}).filter(colName => !!columnsMap[colName]);
        const result = dataArray.reduce((acc, item, index, arr) => {
            ...
            if (acc.groupColumnSummaries !== undefined) {
                acc.groupColumnSummaries = columnsWithSummaries.reduce((acc, colName) => {
            ...
            groupSummary: config.groupSummaryReducer && config.groupSummaryReducer.reducer
        ...
        if (result.groupColumnSummaries) {
            result.groupColumnSummaries = Object.keys(result.groupColumnSummaries).reduce((acc, colName) => {
