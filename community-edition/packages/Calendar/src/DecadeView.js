...
const prepareMinMax = function (props) {
    ...
    const result = {};
    if (minDate != null) {
        result.minDateMoment = toMoment(props.minDate, props).startOf(props.adjustMinDateStartOf);
        result.minDate = +result.minDateMoment;
    }
    if (maxDate != null) {
        result.maxDateMoment = toMoment(props.maxDate, props).endOf(props.adjustMaxDateStartOf);
...
export default class DecadeView extends Component {
    ...
    render() {
        ...
        if (props.navigation) {
            ...
            children = [
                this.renderNav(-1),
                React.createElement(Flex, { key: "year_view", inline: true, flex: true, column: true, alignItems: "stretch", children: children }),
                this.renderNav(1),
            ];
        }
    ...
    renderNav(dir) {
        ...
        const { rootClassName } = props;
        const className = join(`${rootClassName}-arrow`, `${rootClassName}-arrow--${name}`, disabled && `${rootClassName}-arrow--disabled`);
    ...
    renderYear(dateMoment) {
        ...
        const { rootClassName } = props;
        const className = join(`${rootClassName}-year`, isActiveDate && `${rootClassName}-year--active`, isValue && `${rootClassName}-year--value`, props.minDate != null &&
            timestamp < props.minDate &&
            `${rootClassName}-year--disabled`, props.maxDate != null &&
