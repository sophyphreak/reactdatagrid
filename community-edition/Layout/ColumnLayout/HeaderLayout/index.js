...
const getParentsForColumns = (columns, groups, maxDepth) => {
    const parentsForColumns = columns.map(col => {
        const result = [
            col.id,
            ...getParentGroups(col.group, groups, { includeSelf: true }).map(g => g.name),
        ];
...
const getValidDropPositions = ({ dragTargetDepth, dragTargetIndex, dragTargetLength, parentsForColumns, columns, allowGroupSplitOnReorder, }) => {
    ...
    if (!allowGroupSplitOnReorder) {
        const dragTargetParentGroups = getGroupsForColumn(parentsForColumns[dragTargetIndex]).filter(x => x);
        ...
        const currentGroupEndIndex = getGroupEndFor(parentsForColumns, usedDepth, dragTargetIndex);
        for (let i = 0; i <= columns.length; i++) {
            delete res[i];
        }
        for (let i = parentGroupStartIndex; i < parentGroupEndIndex; i++) {
            const itGroupStartIndex = getGroupStartFor(parentsForColumns, usedDepth, i);
        ...
        const groupEndIsLastUnlocked = columns[currentGroupEndIndex] &&
...
class ReorderArrow extends React.Component {
    ...
    render() {
        ...
        return (React.createElement("div", { ref: this.refReorderArrow, style: {
                ...
                opacity: visible ? 1 : 0,
                display: 'block',
            }, className: `InovuaReactDataGrid__column-reorder-arrow InovuaReactDataGrid__column-reorder-arrow--direction-${this.props.rtl ? 'rtl' : 'ltr'}` },
            React.createElement("div", { className: `InovuaReactDataGrid__column-reorder-arrow-fill InovuaReactDataGrid__column-reorder-arrow-fill--direction-${this.props.rtl ? 'rtl' : 'ltr'} InovuaReactDataGrid__column-reorder-arrow-fill--target-${target ||
                    'none'}` }),
...
export default class InovuaDataGridHeaderLayout extends Component {
    constructor(props) {
        ...
        this.lazyNotifyHeaderScrollLeftMax = debounce(this.notifyHeaderScrollLeftMax, 150);
    }
    ...
    renderGroupToolbar() {
        const { columnsMap, onGroupByChange, onHeaderSortClick, renderGroupItem, renderSortTool, renderGroupToolbar, computedGroupBy: groupBy, disableGroupByToolbar, i18n, rtl, theme, } = this.props;
    ...
    renderHeaderWrapper() {
        ...
        const scrollLeft = (this.scrollLeft !== undefined ? this.scrollLeft : props.scrollLeft) || 0;
        const groupBy = this.props.computedGroupBy && this.props.computedGroupBy.length
            ? this.props.computedGroupBy
            : null;
        return (React.createElement(HeaderWrapper, { ...props, groupBy: groupBy, onHeaderGroupMouseDown: this.onHeaderGroupMouseDown, onHeaderCellMouseDown: this.onHeaderCellMouseDown, onHeaderCellTouchStart: this.onHeaderCellTouchStart, ref: this.headerWrapper, refHeader: this.refHeader, scrollLeft: scrollLeft, setScrollLeft: this.setScrollLeft }));
    }
    renderDragCell() {
        return React.createElement(DragCell, { ref: this.refDragCell });
    }
    renderDragGroupItem() {
    ...
    renderDragHeaderGroup() {
        ...
        return React.createElement(DragHeaderGroup, { ref: this.refDragHeaderGroup });
    }
    ...
    onHeaderGroupMouseDown(event, headerGroupProps, headerGroup) {
        ...
        const dragTargetNode = headerGroup.domRef
            ...
            : null;
        const shouldStop = [
            ...dragTargetNode.querySelectorAll('.InovuaReactDataGrid__column-header__filter-wrapper'),
        ].reduce((shouldStop, filterWrapperNode) => {
            if (contains(filterWrapperNode, event.target) ||
        ...
        const parentsForColumns = getParentsForColumns(this.props.visibleColumns, this.props.computedGroupsMap, this.props.computedGroupsDepth);
    ...
    onHeaderCellMouseDown({ computedVisibleIndex: visibleIndex }, event, { dragTarget = 'header', dragIndex } = { dragTarget: 'header' }) {
        ...
        if (event.nativeEvent.which == 3 /* right click */ ||
            event.metaKey ||
            event.ctrlKey) {
            return;
        }
        ...
        if (dragTarget == 'header') {
            ...
            if (this.props.computedGroupBy) {
                if (this.props.computedGroupBy.indexOf(dragColumn.id) != -1 ||
        ...
        const dragTargetDepth = dragColumn &&
            dragColumn.group &&
    ...
    maybeDragMouseDown(event, { dragTarget, dragIndex, allowTargetChange, ranges, parentsForColumns, dragTargetIndex, dragTargetDepth, dragTargetNode, dragTargetLength, }) {
    ...
    setReorderArrowAt(index, ranges, target, offset = 0, visible) {
        ...
        return this.setReorderArrowPosition(rtl
            ? -boxPos + headerRegion.right - offset
            : boxPos - headerRegion.left + offset, target);
    }
    ...
    setupDrag({ dragTarget, dragIndex, allowTargetChange = true, parentsForColumns, dragTargetIndex, dragTargetDepth, dragTargetLength, }, event) {
        const columns = this.props.visibleColumns;
        const { rtl } = this.props;
        const headerRegion = Region.from(this.headerDomNode.current);
        const dragBox = this.getDragBoxInstance(dragIndex, dragTarget, dragTargetDepth, dragTargetLength);
        const initialScrollLeft = this.getScrollLeft();
        const dragBoxProps = dragBox.getProps ? dragBox.getProps() : dragBox.props;
        const rtlOffset = Math.max(this.props.totalComputedWidth - this.props.size.width, 0);
        ...
        const dragColumn = dragTarget === 'group' ? dragBoxProps.column : columns[dragIndex];
        const headerDragColumn = dragTarget == 'header' ? dragColumn : null;
        const columnId = dragTarget == 'header'
            ? headerDragColumn.id
            : dragTarget == 'group'
                ? dragColumn.id
                : null;
        ...
        if (this.props.computedFilterable) {
            const filterWrapperNode = wrapperNode.querySelector('.InovuaReactDataGrid__column-header__filter-wrapper');
        ...
        DRAG_INFO.reorderProxyPosition = {
            group: {
                ...
                bottom: groupTargetNode.offsetHeight -
        ...
        }, { onDrag: this.onHeaderCellDrag, onDrop: this.onHeaderCellDrop }, event);
    }
    onHeaderCellDrop() {
        ...
        const { dragColumn, columnId, dragColumnsIds, dragTarget, dropTarget, dragProxy, newLocked, } = DRAG_INFO;
    ...
    ungroupColumnToIndex(columnId, dropIndex) {
        ...
        const columnAtDropIndexOrLast = columnAtDropIndex || visibleColumns[visibleColumns.length - 1];
    ...
    moveColumnsBeforeIndex(columnsIds, dropIndex, newLocked) {
        const { props } = this;
        const { visibleColumns, columnVisibilityMap, allowGroupSplitOnReorder, } = props;
        const columnOrder = getColumnOrder(props);
        const columns = visibleColumns.filter(col => columnsIds.indexOf(col.id) != -1);
        // moving a column to another index
        const dragColumnsIndexesInAllColumns = columnsIds.map(columnId => columnOrder.indexOf(columnId));
        ...
        const targetColumn = visibleColumns[dropIndex] || visibleColumns[visibleColumns.length - 1];
        columns.forEach(col => {
            if (newLocked != col.computedLocked) {
                this.props.setColumnLocked(col.id, newLocked);
            }
        });
        const newColumnOrder = moveXBeforeY(columnOrder, dragColumnsIndexesInAllColumns, dropBeforeIndexInAllColumns);
    ...
    onHeaderCellDrag({ left: diffLeft, top: diffTop }) {
        ...
        const { allowTargetChange, dragTarget, dragIndex, dragProxy, dragColumn, dragBoxInitialRegion, dragBoxOffsets, dragBoxRegion, groupByRanges, columnRanges: theRanges, headerRegion, scrollLeftMax, initialScrollLeft, validDropPositions, } = DRAG_INFO;
        dragBoxRegion.set({
        ...
        const groupToolbarNode = this.groupToolbar &&
        ...
        if (this.groupToolbar.current && allowTargetChange) {
            ...
            if (dragBoxRegion.top + dragBoxRegion.height / 2 <
        ...
        if (dropTarget == 'header' || dropTarget == 'headergroup') {
            ...
            else if (dragBoxInitialRegion.right + initialDiffLeft >
        ...
        if (rtl) {
            dragProxy.setRight(dragBoxOffsets.right +
                -initialDiffLeft -
                dragBoxInitialRegion.right -
                dragProxyAdjust);
        }
        else {
            dragProxy.setLeft(dragBoxInitialRegion.left +
                initialDiffLeft -
                dragBoxOffsets.left -
                dragProxyAdjust);
        }
        dragProxy.setTop(dragBoxRegion.top - dragBoxOffsets.top);
        let dir = dropTarget == 'group' && dragTarget == 'group'
            ? initialDiffLeft < 0
                ? -1
                : 1
        ...
        const dragMinIndex = dropTarget == 'header' ? DRAG_INFO.minHeaderIndex : undefined;
        const dragMaxIndex = dropTarget == 'header' ? DRAG_INFO.maxHeaderIndex : undefined;
        let dropIndex;
        ...
        if (newDropIndex === this.props.firstLockedEndIndex &&
