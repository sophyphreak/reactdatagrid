...
export default class DataGridLoadMask extends React.Component {
    ...
    render() {
        ...
        const className = join(DEFAULT_CLASSNAME, props.className, livePagination && `${DEFAULT_CLASSNAME}--live-pagination`);
        return (React.createElement(LoadMask, { ...cleanProps, ref: this.maskRef, style: style, visible: visible, className: className, onWheel: livePagination ? stop : null }, renderEmptyContent(loadingText, 'loading')));
    }
}
