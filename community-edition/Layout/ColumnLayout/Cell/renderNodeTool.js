...
export default ({ render, nodeLoading, nodeCollapsed, leafNode, node, nodeProps, toggleNodeExpand, style, size, renderTreeCollapseTool, renderTreeExpandTool, renderTreeLoadingTool, }, cellProps) => {
    ...
    const renderNodeLoading = ({ domProps, size, }) => {
        ...
        if (renderTreeLoadingTool === null) {
            return React.createElement("div", { style: domProps.style });
        }
        if (renderTreeLoadingTool) {
            loadingTool = renderTreeLoadingTool({ domProps, size, className });
        }
        if (loadingTool === undefined) {
            loadingTool = (React.createElement("svg", { ...domProps, className: className, height: size, width: size, viewBox: "0 0 24 24" },
