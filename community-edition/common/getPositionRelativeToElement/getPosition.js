...
// position
function getPosition({ constrainTo = true, targetNode, overlayNode, offset = 0, positions = posiblePositions, relativeToViewport = true, arrowSize = 10, adjustOnPositionBottom = true, showArrow = true, }) {
    ...
    const succesfullPosition = newRegion.alignTo(alignRegion, positionsNormalized, {
        constrain,
    ...
    if (isPositionBottom(succesfullPosition) && adjustOnPositionBottom) {
        position.bottom = -(overlayRegion.height +
