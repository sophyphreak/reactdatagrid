...
class InovuaDropDownButton extends Component {
    ...
    render() {
        const { props } = this;
        const className = join(props.rootClassName, props.className, props.theme && `${props.rootClassName}--theme-${props.theme}`, this.state.focused && `${props.rootClassName}--focused`, props.disabled && `${props.rootClassName}--disabled`, this.getExpanded() && `${props.rootClassName}--expanded`, props.rtl ? `${props.rootClassName}--rtl` : `${props.rootClassName}--ltr`);
        return (React.createElement("div", { ...cleanProps(props, InovuaDropDownButton.propTypes), ref: this.rootRef, className: className },
            this.renderButton(),
            this.renderMenu()));
    }
    renderButton() {
        ...
        if (result === undefined) {
            result = React.createElement(Button, { ...buttonProps });
        }
        return result;
    }
    handleClick(event) {
    ...
    renderMenu(props = this.props) {
        ...
        if (result === undefined) {
            result = React.createElement(MenuComp, { ...menuProps });
        }
        return result;
    }
    renderArrow() {
        ...
        if (result === null) {
            result = (React.createElement(ToggleIcon, { key: "icon", className: `${props.rootClassName}__arrow`, expanded: this.getExpanded() }));
        }
        return result;
    }
    getCommonProps() {
