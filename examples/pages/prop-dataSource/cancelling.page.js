...
const loadData = ({ skip, sortInfo, limit, columnNames }) => {
    ...
    return fetch(DATASET_URL +
        ...
        '&columns=' +
        columnNames.join(',')).then(response => {
...
const App = () => {
    ...
    const dataSource = useCallback(params => {
        ...
        return loadData(params).then(result => {
            ...
            const data = result.data.map(d => Object.fromEntries(Object.entries(d).filter(([key]) => columnKeys.includes(key))));
    ...
    return (React.createElement("div", null,
        ...
        React.createElement(ReactDataGrid, { idProperty: "id", style: gridStyle, columns: tableColumns, licenseKey: process.env.NEXT_PUBLIC_LICENSE_KEY, pagination: true, livePagination: true, limit: 10, dataSource: dataSource })));
};
