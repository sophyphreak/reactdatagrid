...
class InovuaOverlay extends Component {
    ...
    render() {
        ...
        return (React.createElement("div", { ...cleanProps(props, InovuaOverlay.propTypes), className: className, ref: this.rootRef, style: style, onKeyDown: this.handleKeyDown },
            React.createElement("div", { className: contentClassName, ...this.getChildrenProps(), style: contentStyle }),
            props.arrow && this.renderArrow(),
            props.relativeToViewport && this.renderFixed()));
    }
    renderFixed() {
        return (React.createElement("div", { style: { position: 'fixed', top: 0, left: 0, width: 0, height: 0 }, ref: this.fixedRef }));
    }
    renderArrow() {
        ...
        const className = join(`${rootClassName}__arrow`, `${rootClassName}__arrow--${location}`, this.props.arrowClassName);
        ...
        const wrapperClassName = join(`${rootClassName}__arrow-wrapper`, `${rootClassName}__arrow-wrapper--${location}`);
        return (React.createElement(Arrow, { className: className, wrapperClassName: wrapperClassName, position: position, style: style, size: this.props.arrowSize }));
    }
    /**
    ...
    prepareClassName() {
        ...
        const { rootClassName, theme, visibleClassName, relativeToViewport, } = props;
        const visible = this.getVisible();
        let className = join(rootClassName, props.className, visible && `${rootClassName}--visible`, visible && visibleClassName, !visible && `${rootClassName}--invisible`, relativeToViewport && `${rootClassName}--position-fixed`, theme && `${rootClassName}--theme-${theme}`);
        if (this.props.fade) {
            // animation
            className = join(className, props.fade && `${rootClassName}--has-transition`, state.transitionEnter && `${rootClassName}--transition-enter`, state.transitionEnterActive &&
                `${rootClassName}--transition-enter-active`, state.transitionLeave && `${rootClassName}--transition-leave`, state.transitionLeaveActive &&
                `${rootClassName}--transition-leave-active`);
        }
        return className;
    }
    prepareContentClassName() {
        return join(this.props.contentClassName, `${this.props.rootClassName}__content`);
    }
    prepareStyle() {
        const style = assign({
    ...
    getPositionConfig() {
        ...
        if (tooltipPositions) {
            positions = tooltipPositions
                .split(',')
                .map(position => position && position.trim ? position.trim() : position);
        }
