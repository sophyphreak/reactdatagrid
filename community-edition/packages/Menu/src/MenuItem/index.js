...
class MenuItem extends Component {
    ...
    render() {
        const props = (this.preparedProps = this.prepareProps(this.props, this.state));
        const className = props.className;
        return (React.createElement("tr", { ...cleanProps(props, MenuItem.propTypes), style: props.style, className: className, ref: node => (this.node = node) }, this.renderCells()));
    }
    componentDidMount() {
    ...
    renderSelectInput() {
        ...
        const className = join(`${props.rootClassName}__cell__input`, props.browserNativeSelectInputs &&
            `${props.rootClassName}__cell__input--browser-native`, multiple && `${props.rootClassName}__cell__input--multiple`);
        ...
        cellProps.className = join(cellProps.className, `${props.rootClassName}__cell--has-input`, props.multiple && `${props.rootClassName}__cell--checkbox`, !props.multiple && `${props.rootClassName}__cell--radio`);
        ...
        if (result === undefined) {
            result = React.createElement(Input, { ...inputProps });
        }
        return React.createElement(MenuItemCell, { ...cellProps }, result);
    }
    renderExpander() {
        const { props } = this;
        ...
        if (expander === undefined || expander === true) {
            expander = React.createElement(Expander, { ...expanderProps });
        }
        return (React.createElement(MenuItemCell, { key: "expander", className: `${props.rootClassName}__cell--has-expander`, expander: expander, ...expanderProps }));
    }
    getStyle() {
        ...
        if (state.active) {
            assign(style, props.itemActiveStyle, props.activeStyle, props.item.activeStyle);
        }
        if (props.expanded) {
        ...
        if (props.disabled) {
            assign(style, props.itemDisabledStyle, props.disabledStyle, props.item.disabledStyle);
        }
        return style;
    }
    getClassName() {
        ...
        let className = join(props.className, props.item.className, baseClassName, state.mouseOver && `${baseClassName}--over`, state.mouseOver && props.item.overClassName, state.mouseOver && props.overClassName, state.active && `${baseClassName}--active`, state.active && props.item.activeClassName, state.active && props.activeClassName, props.expanded && `${baseClassName}--expanded`, props.expanded && props.item.expandedClassName, props.expanded && props.expandedClassName, props.focused && `${baseClassName}--focused`, props.focused && props.item.focusedClassName, props.focused && props.focusedClassName, props.item.isTitle && `${baseClassName}--title`, props.checked && `${baseClassName}--checked`);
        // when disabled only disabled className is applied
        if (props.disabled) {
            className = join(props.className, baseClassName, props.disabled && `${baseClassName}--disabled`, props.disabled && props.itemDisabledClassName, props.disabled && props.item.disabledClassName);
        }
        return className;
    }
    getOffset() {
