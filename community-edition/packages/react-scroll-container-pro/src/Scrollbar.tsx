...
export default class InovuaScrollbar extends Component {
  ...
  render() {
    ...
    className = join(
      ...
      `inovua-react-scroll-container__scrollbar--orientation-${orientation}`,
      `inovua-react-scroll-container__scrollbar--direction-${
        this.props.rtl ? 'rtl' : 'ltr'
      }`,
      !thumbVisible && 'inovua-react-scroll-container__scrollbar--hidden',
      hiding && 'inovua-react-scroll-container__scrollbar--hiding',
      this.state.active && 'inovua-react-scroll-container__scrollbar--active',
      showing && 'inovua-react-scroll-container__scrollbar--showing'
    );
    ...
    const scrollThumbOverWidth = Math.max(
      this.props.scrollThumbOverWidth,
      this.props.scrollThumbWidth
    );

    const thumbRadius =
      this.props.scrollThumbRadius !== undefined
        ? this.props.scrollThumbRadius
        : this.props.scrollThumbWidth;
    const thumbStyle = Object.assign({}, this.props.scrollThumbStyle, {
      [sizeName]: thumbSize,
      [otherSizeName]:
        this.state.over || this.state.active || this.props.alwaysShowTrack
    ...
    const thumbClassName = `inovua-react-scroll-container__thumb inovua-react-scroll-container__thumb--orientation-${orientation} inovua-react-scroll-container__thumb--direction-${
      this.props.rtl ? 'rtl' : 'ltr'
    }`;
    let trackClassName = `inovua-react-scroll-container__track inovua-react-scroll-container__track--orientation-${orientation} inovua-react-scroll-container__track--direction-${
      this.props.rtl ? 'rtl' : 'ltr'
    }`;

    const trackVisible =
      this.props.alwaysShowTrack ||
    ...
    const onThumbMouseDown =
      thumbVisible && this.props.dragToScroll ? this.onThumbMouseDown : null;
    const onTrackClick =
      thumbVisible && this.props.dragToScroll ? this.onTrackClick : null;
    const onTrackWheel =
      this.props.dragToScroll && this.state.over ? this.onTrackWheel : null;
  ...
  getThumbPosition() {
    return this.scrollPosToThumbPos(
      this.getScrollPos(),
      this.orientation,
      this.getThumbSize()
    );
  ...
  onMouseMove(event) {
    ...
    const scrollPos = this.thumbPosToScrollPos(
      this.initialThumbPos + diff,
      this.orientation,
      this.thumbSize
    );
  ...
  getThumbTransform(
    ...
    let scrollPos =
      this.scrollPosToThumbPos(this.getScrollPos(), orientation, thumbSize) ||
      0;
  ...
  thumbPosToScrollPos(thumbPos, orientation, thumbSize) {
    ...
    const scrollPos =
      (thumbPos * (scrollSize - clientSize)) / (trackSize - thumbSize);
  ...
  scrollPosToThumbPos(scrollPos, orientation, thumbSize) {
    ...
    const thumbPos =
      (scrollPos / (scrollSize - clientSize)) * (trackSize - thumbSize);
