...
class Value extends Component {
    ...
    render() {
        const { props } = this;
        const { value, multiple, rootClassName, toolsSize, focus, rtl, searchable, wrapMultiple, label, } = props;
        const showTags = value != null && multiple;
        const style = { ...props.style };
        if (toolsSize) {
            style.maxWidth = `calc(100% - ${toolsSize.width}px)`;
        }
        const className = join(rootClassName, wrapMultiple ? `${rootClassName}--wrap` : `${rootClassName}--no-wrap`, `${rootClassName}--${rtl ? 'rtl' : 'ltr'}`);
    ...
    renderTags() {
        ...
        const remainingTags = remainingItems &&
            this.renderTag(remainingItems, this.props.maxTagsLength, {
                visibleItems,
            });
        const visibleTags = visibleItems &&
    ...
    renderTag(item, index, config = {}) {
        ...
        const { rootClassName, renderTag, activeTag, maxTagsLength, renderTagLabel, renderRemainingTags, } = this.props;
        ...
        if (tag === undefined) {
            tag = React.createElement(Tag, { ...tagProps });
        }
        return tag;
    }
}
