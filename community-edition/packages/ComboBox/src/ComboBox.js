...
import { getPageCount, getCurrentPage, } from '../../PaginationToolbar';
...
class InovuaComboBox extends Component {
    ...
    render() {
        ...
        const groupedItems = (this.groupedItems = this.getGroupedItems(this.getSelectedItems()));
        ...
        return (React.createElement("div", { ...cleanProps(props, InovuaComboBox.propTypes), onClick: this.handleComboClick, onMouseDown: this.handleComboMouseDown, onKeyDown: this.handleComboKeyDown, onFocus: this.handleComboFocus, onBlur: this.handleComboBlur, className: className, style: style, ref: this.addRootRef, onMouseEnter: this.handleMouseEnter, onMouseLeave: this.handleMouseLeave },
            ...
            React.createElement("div", { className: `${props.rootClassName}__tools`, ref: this.refTools },
                ...
                this.props.tagEllipsis && (React.createElement(NotifyResize, { notifyOnMount: true, onResize: this.handleToolsSize }))),
    ...
    renderList() {
        ...
        if (result === undefined) {
            result = React.createElement(List, { ...listProps });
        }
    ...
    renderClearIcon() {
        ...
        const clearButton = (React.createElement(ClearButton, { onClear: this.clear, closeIcon: props.clearIcon, className: `${props.rootClassName}__clear-icon` }));
        return clearButton;
    }
    renderToggleIcon() {
        const { props } = this;
        ...
        return (React.createElement(ToggleButton, { onToggle: this.toggleExpand, className: `${props.rootClassName}__toggle-icon`, toggleIcon: props.toggleIcon, expanded: this.getExpanded() }));
    }
    // renders
    renderTextInput() {
        ...
        let suggestion = null;
        if (this.props.minAutocompleteLength <= (value && value.length) &&
            data &&
            data[0]) {
        ...
        if (result === undefined) {
            result = React.createElement(TextInput, { ...inputProps });
        }
        return result;
    }
    renderSpinner() {
        ...
        const spinner = this.props.loadingSpinner === true ? (React.createElement(LoadingIcon, { className: `${this.props.rootClassName}__loading-spinner` })) : (this.props.loadingSpinner);
        return loading
            ? spinner
    ...
    getListProps() {
        const { props, state } = this;
        const data = this.getFilteredData();
        const groupsLength = (this.state.groups && Object.keys(this.state.groups).length) || 0;
    ...
    // data
    loadDataSource(dataSource, props = this.props, { appendTo, action, value, text, skip, limit, hasNextPage } = emptyObject) {
    ...
    remoteFilterData({ text = this.getText(), value = this.getValue(), filterType, } = {}) {
    ...
    updateFilteredData({ text = this.getText(), value = this.getValue(), data = this.state.data, filterType, } = {}) {
        ...
        const isFilteredByText = filterType === 'text' && data.length !== filteredData.length;
    ...
    handleTextChange(text) {
        ...
        this.setText(text, { skipFilter: this.props.expandOnTextChange && !this.getExpanded() }, () => {
            /*
    ...
    removeRemainingTags() {
        ...
        if (activeTag === REMAINING_ITEMS) {
            const lastItem = Array.isArray(visibleItems) && visibleItems[visibleItems.length - 1];
            if (lastItem) {
    ...
    handleEnterKeyPress(event) {
        ...
        if (this.props.navigateToNextAfterSelection &&
            this.props.multiple &&
            activeItem) {
            this.navigateToNextItem(1, event);
        }
    ...
    isNavigationAllowed(direction) {
        ...
        const currentActiveTag = this.getActiveTag();
        const canNavigateLeft = inputNode.isCursorAtStartPosition() && direction === -1;
        const canNavigateRight = inputNode.isCursorAtEndPosition() && direction === 1;
    ...
    handleTagBackspaceRemove(event) {
        ...
        const inputNode = this.getTextInputNode();
        const canDeleteTag = value && value.length && inputNode.isCursorAtStartPosition();
        if (canDeleteTag) {
            /**
    ...
    handleToolsSize(size = this.toolsNode
    ...
    updateGroups({ groupProperty = this.props.groupProperty, data = this.getFilteredData(), }) {
        if (!data) {
            return null;
        }
