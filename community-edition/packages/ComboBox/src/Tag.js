...
class Tag extends Component {
    ...
    render() {
        const { props } = this;
        const { item, index, items, rootClassName, closeIconPosition, onClick, active, multiple, renderTag, isMultiple, } = props;
        ...
        const domProps = {
            ...
            children: [
                ...
                React.createElement("span", { key: "tag_label", className: labelClassName }, label),
    ...
    getClassName({ active }) {
        ...
        const className = join(rootClassName, active && `${rootClassName}--active`, ellipsis && `${rootClassName}--ellipsis`, isMultiple && `${rootClassName}--multiple`);
        return className;
    }
    getStyle() {
        const { props } = this;
        const { border, padding, height, width, maxWidth, activeStyle, active, } = props;
    ...
    renderLabel() {
        const { index, maxTagsLength, renderTagLabel, items, item, isMultiple, } = this.props;
    ...
    renderCloseIcon() {
        ...
        if (typeof closeIcon === 'function') {
            ...
            if (closeIcon && typeof closeIcon == 'object') {
                closeIconEl = cloneElement(closeIcon, {
                    className: join(closeIcon.props && closeIcon.props.className, closeIconProps.className),
                    onClick: closeIcon.props && closeIcon.props.onClick
                        ? event => {
        ...
        if (closeIconEl === true || closeIconEl === undefined) {
            closeIconEl = React.createElement(CloseIcon, { ...closeIconProps });
        }
        return closeIconEl;
    }
