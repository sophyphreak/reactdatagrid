...
export default class InovuaVirtualList extends Component {
    ...
    renderScrollerSpacerOnNaturalRowHeight = (spacerProps) => {
    ...
    render() {
        ...
        return (React.createElement(Factory, { contain: this.props.contain, ResizeObserver: this.props.ResizeObserver !== undefined
    ...
    setRowIndex = (row, index, callback) => {
    ...
    componentDidUpdate(prevProps) {
        ...
        const rowHeightChange = prevProps.rowHeight && this.props.rowHeight != prevProps.rowHeight;
        if (prevProps.renderRow !== this.props.renderRow) {
            this.rowCoveredBy = {};
            this.rowSpans = {};
        }
        if (nextCount != prevProps.count ||
            this.props.showEmptyRows != prevProps.showEmptyRows ||
            rowHeightChange) {
    ...
    setRowRowSpan = (rowIndex, rowSpan) => {
        if (rowSpan === 1) {
            return;
        }
    ...
    currentStickyRows = [];
    updateStickyRows = (scrollTop = this.scrollTop, firstVisibleRowIndex, { force } = { force: true }) => {
    ...
    setStickyRows = (currentStickyRows = this.currentStickyRows, scrollTop = this.scrollTop, enteringRow) => {
