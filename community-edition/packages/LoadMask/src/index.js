...
const InovuaLoadMask = forwardRef((props, ref) => {
    const visibleClassName = props.visible
        ...
        : '';
    const className = join(props.className, props.rootClassName, visibleClassName, props.theme && `${props.rootClassName}--theme-${props.theme}`);
    const layerClassName = join(props.backgroundLayerClassName, `${props.rootClassName}__background-layer`);
    ...
    return (React.createElement("div", { ref: ref, ...cleanProps(props, InovuaLoadMask.propTypes), className: className, style: style },
        React.createElement("div", { style: layerStyle, className: layerClassName }),
        React.createElement("div", { className: `${props.rootClassName}__loader-container` },
            React.createElement(Loader, { size: props.size, theme: props.theme, animationDuration: props.animationDuration }),
            props.children)));
});
