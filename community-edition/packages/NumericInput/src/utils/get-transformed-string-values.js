...
export default function getTransformedStringValues(stringInput, props = {}, internalApi = {}) {
    ...
    const { locale, min = Number.MIN_SAFE_INTEGER, max = Number.MAX_SAFE_INTEGER, suffix, prefix, } = props;
    let precision = props.precision || null;
    if (precision > 20) {
        precision = 20;
    }
    const { getDecimalDelimiter = internalGetDecimalDelimiter, getDigitGroupDelimiter = internalDigitGroupDelimiter, convertStringToNumber = internalConvertStringToNumber, } = internalApi;
    ...
    let decimalDigitsInInput = countDecimalDigits(stringInput, decimalDelimiter, decimalDelimiterPosition);
