...
export const renderColumnFilterContextMenu = (
  ...
  const columnOperators =
    computedProps.initialProps.filterTypes![filterValue.type].operators ||
    emptyArray;
  ...
  const onSelectionChange = (selected: any) => {
    ...
    const operatorDescriptor = filterTypeDescription.operators.filter(
      op => op.name === operator
    )[0];
  ...
  const menuProps = {
    ...
    alignPositions:
      computedProps.columnFilterContextMenuAlignPositions || rtl
  ...
  if (computedProps.initialProps.renderColumnFilterContextMenu) {
    result = computedProps.initialProps.renderColumnFilterContextMenu!(
      menuProps,
      {
        cellProps,
...
const useFilterValue = (
  ...
  },
  {
    columnsMap,
    enableFiltering,
  }: { columnsMap: TypeComputedColumnsMap; enableFiltering?: boolean },
  computedPropsRef: MutableRefObject<TypeComputedProps | null>
): [TypeFilterValue, (filterValue: TypeFilterValue) => any] => {
  let [filterValue, doSetFilterValue]: [TypeFilterValue, any] = useProperty<
    TypeFilterValue
  >(props, 'filterValue');
  ...
  const setFilterValue = useCallback(
    (filterValue: TypeFilterValue) => {
      ...
      filterValue = getFilterValueForColumns(
        filterValue,
        computedProps.columnsMap
      );
...
const useFilters = (
  ...
  const [enableFiltering, setEnableFiltering] = useProperty<boolean>(
    props,
    'enableFiltering'
  );
  ...
  const getMenuAvailableHeight = useCallback((): number | undefined => {
    const { current: computedProps } = computedPropsRef;
    if (!computedProps) {
      return undefined;
    }
  ...
  const clearAllFilters = useCallback(() => {
    ...
    const allFiltersValue: TypeSingleFilterValue[] = [];
  ...
  const columnFilterContextMenuAlignToRef = useRef<any>(null);
  const columnFilterContextMenuConstrainToRef = useRef<any>(null);
  const [
  ...
  const computedFilterValueMap: {
    ...
    return computedFilterValue
      ? computedFilterValue.reduce((acc, columnFilterValue) => {
          ...
          return acc;
        }, {} as { [key: string]: TypeSingleFilterValue })
      : null;
  }, [computedFilterValue]);

  const computedOnColumnFilterValueChange = useCallback(
    (columnFilterValue: {
      ...
      const newFilterValue = (computedFilterValue || [])
        .map(oldFilterValue => {
          if (oldFilterValue.name === columnFilterValue.columnId) {
            const emptyValue = getColumnFilterEmptyValue(
              columnFilterValue.columnId
            );

            if (
              columnFilterValue.filterValue.type === 'date' ||
              columnFilterValue.filterValue.type === 'number'
            ) {
              if (
                isBinaryOperator(oldFilterValue.operator) &&
  ...
  const getColumnFilterValue = useCallback(
    (column: TypeGetColumnByParam): TypeSingleFilterValue | null => {
      const { current: computedProps } = computedPropsRef;
      if (!computedProps) {
        return null;
      }
  ...
  const setColumnFilterValue = useCallback(
    (column: TypeGetColumnByParam, value: any, operator?: string) => {
      const { current: computedProps } = computedPropsRef;
      if (!computedProps) {
        return null;
      }
      ...
      const col = computedProps.getColumnBy(column) as TypeComputedColumn;
  ...
  const getColumnFilterEmptyValue = useCallback(
  ...
  const clearColumnFilter = useCallback(
    (idNameOrIndex: TypeGetColumnByParam) => {
      ...
      computedProps.setColumnFilterValue(
        idNameOrIndex,
        getColumnFilterEmptyValue(idNameOrIndex)
      );
    },
    []
  );

  const isFiltered = useCallback((): boolean => {
    const { current: computedProps } = computedPropsRef;
    if (!computedProps) {
      return false;
    }
    return (computedProps.computedFilterValue || []).reduce(
      (result: boolean, filterValue: TypeSingleFilterValue) => {
        if (result) {
          return true;
        }
        const col = computedProps.getColumnBy(
          filterValue.name
