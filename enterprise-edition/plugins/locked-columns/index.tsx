...
import {
  ...
  sumColumnWidth,
} from './updateLockedWrapperPositions';
export default {
  name: 'locked-columns',
  hook: () => {},
  defaultProps: () => {
  ...
  renderLockedStartCells: ({
    ...
    if (lockedStartCount) {
      ...
      if (virtualizeColumns) {
        ...
        let transformStart = `translate3d(${
          rtl && getScrollLeftMax
      ...
      const lockedStartInTransition = addTransitionDuration(
        lockedStartColumns,
        startWrapperStyle
      );
      ...
      return (
        <div
          ...
          className={`InovuaReactDataGrid__locked-start-wrapper ${
            lockedStartInTransition
              ? 'InovuaReactDataGrid__locked-start-wrapper--transition'
              : ''
          } ${
            sticky ? 'InovuaReactDataGrid__locked-start-wrapper--sticky' : ''
          } InovuaReactDataGrid__locked-start-wrapper--direction-${
            rtl ? 'rtl' : 'ltr'
          }`}
  ...
  renderLockedEndCells: ({
    ...
    const endWrapperWidth =
      groupProps && expandGroupTitle
        ? 0
    ...
    const lockedEndInTransition = addTransitionDuration(
      lockedEndColumns,
      endWrapperStyle
    );

    const lockEndActiveBorderDiv = (
      <div
        key="--active-borders"
    ...
    return (
      <div
        ...
        className={`InovuaReactDataGrid__locked-end-wrapper InovuaReactDataGrid__locked-end-wrapper--direction-${
          rtl ? 'rtl' : 'ltr'
        } ${sticky ? 'InovuaReactDataGrid__locked-end-wrapper--sticky' : ''} ${
          lockedEndInTransition
