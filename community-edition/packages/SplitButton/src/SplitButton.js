...
class InovuaSplitButton extends Component {
    ...
    render() {
        const { props } = this;
        const className = join(props.rootClassName, props.className, props.rtl ? `${props.rootClassName}--rtl` : `${props.rootClassName}--ltr`, this.state.focused && `${props.rootClassName}--focused`, this.getExpanded() && `${props.rootClassName}--expanded`, props.disabled && `${props.rootClassName}--disabled`, props.theme && `${props.rootClassName}--theme-${props.theme}`);
        return (React.createElement("div", { ...cleanProps(props, InovuaSplitButton.propTypes), ref: this.buttonRef, className: className },
            React.createElement(Button, { ...this.getCommonProps(), ...this.getButtonProps() }),
            React.createElement(DropdownButton, { ...this.getCommonProps(), ...this.getDropdownButtonProps() })));
    }
    getCommonProps() {
        const { props } = this;
        return {
    ...
    getButtonProps() {
        ...
        return {
            ...
            className: join(`${props.rootClassName}__button`, props.buttonProps.className),
        };
    }
    getDropdownButtonProps() {
        ...
        return {
            ...
            className: join(`${props.rootClassName}__dropdown-button`, props.menuProps.className),
        };
    }
}
function emptyFn() { }
