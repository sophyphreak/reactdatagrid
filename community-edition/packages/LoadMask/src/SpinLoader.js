...
class InovuaSpinLoader extends React.Component {
    render() {
        ...
        const className = join(props.className, DEFAULT_CLASS_NAME, `${DEFAULT_CLASS_NAME}--spin`, props.theme && `${DEFAULT_CLASS_NAME}--theme-${props.theme}`);
        ...
        const bars = [...Array(12)].map((_, i) => {
            const index = i + 1;
            return (React.createElement("div", { key: index, className: `${LOADBAR_CLASSNAME} ${LOADBAR_CLASSNAME}--${index}` }));
        });
        return (React.createElement("div", { style: style, className: className }, bars));
    }
}
