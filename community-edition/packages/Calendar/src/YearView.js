...
import { prepareDateProps, getInitialState, onKeyDown, onViewDateChange, onActiveDateChange, onChange, navigate, select, confirm, gotoViewDate, } from './DecadeView';
...
export default class YearView extends Component {
    ...
    render() {
        ...
        return (React.createElement(Flex, { inline: true, column: true, ref: this.yearViewRef, alignItems: "stretch", tabIndex: 0, ...flexProps, onKeyDown: this.onKeyDown, className: className }, this.renderMonths(props, monthsInView)));
    }
    ...
    renderMonth(props, dateMoment) {
        ...
        const { rootClassName } = props;
        const className = join(`${rootClassName}-month`, !disabled && isActiveDate && `${rootClassName}-month--active`, !disabled && isValue && `${rootClassName}-month--value`, disabled && `${rootClassName}-month--disabled`);
        ...
        return (React.createElement(Item, { key: monthText, className: className, onClick: onClick }, monthText));
    }
