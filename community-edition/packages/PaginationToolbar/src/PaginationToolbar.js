...
import { FIRST_PAGE, LAST_PAGE, PREV_PAGE, NEXT_PAGE, REFRESH, } from './getIcons';
...
const SPACER = React.createElement("div", { className: `${CLASS_NAME}__spacer` });
...
export default class InovuaPaginationToolbar extends React.Component {
    ...
    render() {
        const { totalCount, theme, rtl, remotePagination, rootClassName, changeButtonStyles, } = this.props;
        ...
        const className = join(this.props.className, CLASS_NAME, theme && `${CLASS_NAME}--theme-${theme}`, `${CLASS_NAME}--${rtl ? 'rtl' : 'ltr'}`, this.props.bordered && `${CLASS_NAME}--bordered`);
        ...
        return (React.createElement(ToolBar, { ...domProps, useTransformOnScroll: false, onKeyDown: stopPropagation, rtl: rtl },
            React.createElement("div", { className: `${CLASS_NAME}__region` },
                firstPage,
                prevPage,
                React.createElement("span", { className: `${CLASS_NAME}__page-text` }, this.props.pageText),
                ' ',
                currentPageInput,
                " ",
                this.props.ofText,
                ' ',
                React.createElement("span", { className: `${CLASS_NAME}__page-count-text`, style: {
                ...
                React.createElement("span", { className: `${CLASS_NAME}__per-page-text` }, this.props.pageSizes === false ? null : this.props.perPageText),
                ' ',
                combo,
                this.props.pageSizes !== false && refresh ? React.createElement(Separator, null) : null,
                refresh),
            SPACER,
            React.createElement("div", { className: `${CLASS_NAME}__region` },
    ...
    renderCurrentPageInput({ pageCount, currentPage }) {
        ...
        if (result === undefined) {
            ...
            result = React.createElement(NumericInput, { ...inputProps });
        }
        return result;
    }
    renderPageSizeCombo() {
        ...
        if (result === undefined) {
            result = React.createElement(ComboBox, { ...comboProps, relativeToViewport: true });
        }
        return result;
    }
    renderIcon(name, disabled) {
        ...
        if (result === undefined) {
            result = React.createElement(PaginationIcon, { ...iconProps });
        }
        return result;
    }
    componentDidUpdate = prevProps => {
        ...
        const nextCurrentPage = Math.min(nextPageCount, getCurrentPage({ skip: nextSkip, limit: nextLimit }));
