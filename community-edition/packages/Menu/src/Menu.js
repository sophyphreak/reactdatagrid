...
import { increaseLastColumnColSpan, increaseFirstColumnColSpan, } from './utils/increaseColSpan';
...
import { alignPositionLTR, alignPositionRTL, alignOffsetLTR, alignOffsetRTL, } from './submenuAlignPositions';
...
class InovuaMenu extends Component {
    ...
    render() {
        ...
        const scrollerArrow = props.showScrollArrows === undefined ? 'auto' : !!props.showScrollArrows;
        return (React.createElement("div", { tabIndex: 0, ...cleanProps(props, InovuaMenu.propTypes), ref: this.setRootRef, onMouseLeave: this.handleMouseLeave, onMouseEnter: this.handleMouseEnter, onKeyDown: this.handleKeyDown, onBlur: this.handleOnBlur, className: className, style: style },
            this.state.activeSubMenuIndex !== null &&
                this.renderSubMenu(props, state),
            props.disableScroller ? (table) : (React.createElement(ArrowScroller, { scroller: scrollerArrow, ...this.getScrollerProps() }, table))));
    }
    // RENDERING LOGIC
    renderTable() {
        const { props, state } = this;
        const className = join(`${props.rootClassName}__table`, props.tableClassName);
        return (React.createElement("table", { cellSpacing: 0, cellPadding: 0, className: className, ref: this.setTableRef },
            React.createElement("tbody", null, this.renderItems())));
    }
    renderItems() {
        const { props, state } = this;
        if (!props.items) {
        ...
        return props.items.map((item, index, items) => {
            if (item === '-') {
                return (React.createElement(MenuSeparator, { style: props.menuSeparatorStyle, key: index, rootClassName: props.rootClassName }));
            }
            ...
            const hasSubMenu = (item.items && !!item.items.length) ||
        ...
        if (props.enableSelection) {
            ...
            if (name) {
                selectionProps = {
                    ...
                    renderCheckInput: item.renderCheckInput !== undefined
                        ? item.renderCheckInput
                        : props.renderCheckInput,
                    renderRadioInput: item.renderRadioInput !== undefined
                        ? item.renderRadioInput
                        : props.renderRadioInput,
                    selectionInputPosition: item.selectionInputPosition !== undefined
    ...
    getCommonItemProps() {
        const { props } = this;
        const menuHasSubmenu = !!props.items.filter(item => item.items && item.items.length).length;
    ...
    renderSubMenu(props, state) {
        ...
        const wrapperStyle = getSubMenuStyle.bind(this)({ ...props, alignPositions: this.getDefaultAlignPositions() }, state, domNode);
        const wrapperClassName = `${this.props.rootClassName}__submenu-wrapper`;
        return (React.createElement("div", { className: wrapperClassName, ref: this.setSubMenuWrapRef, style: wrapperStyle, onMouseEnter: this.handleSubMenuMouseEnter, onMouseLeave: this.handleSubMenuMouseLeave },
            React.createElement(InovuaMenu, { key: state.activeSubMenuIndex, ...menuProps })));
    }
    ...
    getSubMenuProps() {
        ...
        if (overridingProps.selected === undefined &&
    ...
    getScrollerProps() {
        const { props, state } = this;
        const className = join(props.scrollerProps.className, `${props.rootClassName}__scroll-container`);
        const style = assign({}, props.scrollerProps.style);
        /**
         * Same height style (height|maxHeight) is used also on scroller
         */
        const maxHeight = state.maxHeight != undefined && props.submenu
            ? state.maxHeight
            : props.maxHeight;
        if (maxHeight != null) {
    ...
    prepareClassName(props, state) {
        ...
        let className = join(props.rootClassName, props.theme && `${props.rootClassName}--theme-${props.theme}`, props.className, !props.visible ||
            (props.items &&
                !props.items.length &&
                `${props.rootClassName}--no-display`), props.absolute && `${props.rootClassName}--absolute`, !props.rtl
            ? `${props.rootClassName}--ltr`
            : `${props.rootClassName}--rtl`, hidden && `${props.rootClassName}--hidden`, isMobile && `${props.rootClassName}--mobile`, props.subMenu && `${props.rootClassName}__submenu`, `${props.rootClassName}--depth-${props.depth}`, state.hasScroll && `${props.rootClassName}--has-overflow`, props.shadow && `${props.rootClassName}--shadow`, state.hasScroll && `${props.rootClassName}--has-scroll`);
        if (props.alignPosition !== undefined) {
            const position = props.alignPosition === 1 ? 'up' : 'down';
            className = join(className, `${props.rootClassName}--position-${position}`);
        }
        // animation
        if (state.enableAnimation) {
            className = join(className, `${props.rootClassName}--animation-enabled`, state.transitionEnded && `${props.rootClassName}--transition-end`, state.transitionStart && `${props.rootClassName}--transition-start`);
        }
        return className;
    }
    // BEHAVIOUR LOGIC
    ...
    handleArrowUp(event) {
        const lastItemIndex = this.props.items && this.props.items.length
            ? this.props.items.length
            : null;
        const { props } = this;
        const focusedItemIndex = this.state.focusedItem != null ? this.state.focusedItem : lastItemIndex;
        const newFocusedItemIndex = getNextNavigableItem(props.items, focusedItemIndex, -1);
    ...
    handleArrowDown(event) {
        const { props } = this;
        const focusedItemIndex = this.state.focusedItem != null ? this.state.focusedItem : -1;
        const newFocusedItemIndex = getNextNavigableItem(props.items, focusedItemIndex, 1);
        if (newFocusedItemIndex == null) {
            return;
        }
    ...
    handleEnterOnFocusedItem(event) {
        ...
        if (this.props.simulateClickOnEnterKeyPress &&
            focusedItem != null &&
    ...
    onSubMenuInactivate(submenuConfig) {
        ...
        this.setState({
            ...
            setTimeout(() => {
                ...
                const mouseHasEnteredSubmenuParentItem = submenuConfig && submenuConfig.parentIndex === this.itemOverIndex;
    ...
    onMenuItemMouseLeave({ leaveOffset, index, event }) {
        ...
        const elementAtMousePosition = globalObject.document.elementFromPoint(leaveOffset.x, leaveOffset.y);
        /**
         ...
         */
        if (elementAtMousePosition === this.subMenuWrap ||
    ...
    onMenuItemClick(event = {}, itemProps, index) {
        ...
        const stopped = event && event.isPropagationStopped
        ...
        if (isMobile &&
            ...
            const menuOffset = getRegionRelativeToParent(event.currentTarget, props.rootClassName);
        ...
        if (!stopped) {
            ...
            const dismissOnClick = itemProps &&
                itemProps.item &&
        ...
        this.setSubMenu = ({ menuOffset, index } = {}, callback) => {
            ...
            if (index != null) {
                if (this.props.showSubMenuDelay) {
                    this.showTimeout = setTimeout(() => setSubMenu({ menuOffset, index }, callback), this.props.showSubMenuDelay);
                }
                else {
                ...
                if (this.props.hideSubMenuDelay) {
                    this.hideTimeout = setTimeout(() => setSubMenu({ menuOffset, index }, callback), this.props.hideSubMenuDelay);
                }
                else {
                    setSubMenu({ menuOffset, index }, callback);
                }
    ...
    setupEnterAnimation() {
        ...
        // submenu correctly
        setTimeout(() => {
            const enableAnimation = (this.props.enableAnimation && this.props.subMenu) ||
        ...
        if ((props.constrainTo || alignTo) && !props.subMenu) {
            const doAlign = () => {
                ...
                if (alignTo) {
                    ...
                    const offsetParentRegion = Region.from(offsetParent || {
                    ...
                    const transform = `translate3d(${Math.floor(newLeft)}px, ${Math.floor(newTop)}px, 0px)`;
                    positionStyle = {
    ...
    getFocusedItem() {
        return (this.state.focusedItem != null &&
