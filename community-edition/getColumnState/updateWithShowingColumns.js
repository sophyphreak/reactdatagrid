...
export default (columns, props, state, context) => {
    context = context || {};
    const showingColumnsMap = assign(context.showingColumnsMap, getShowingColumns(columns, state.columnsMap));
    const showingKeys = Object.keys(showingColumnsMap);
    if (showingKeys.length) {
        const showingColumn = showingColumnsMap[showingKeys[0]];
        const duration = showingColumn.showTransitionDuration !== undefined
