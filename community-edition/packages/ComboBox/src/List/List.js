...
class List extends Component {
    constructor(props) {
        ...
        this.virtualListShouldComponentUpdate = this.virtualListShouldComponentUpdate.bind(this);
    ...
    render() {
        ...
        if (this.state.succesfullPosition && this.props.offset) {
            const positionName = this.state.succesfullPosition === 'bc-tc' ? 'top' : 'bottom';
        ...
        const result = (React.createElement("div", { ...cleanProps(props, List.propTypes), className: className, style: style, ref: this.setRootNode, onClick: this.handleOnClick },
        ...
        if (this.props.relativeToViewport) {
            ...
            return React.createElement(Overlay, { ...overlayProps }, result);
        }
        return result;
    }
    getOverlayTarget() {
    ...
    renderEmptyText() {
        ...
        return (React.createElement("div", { className: `${this.props.rootClassName}__empty-text` }, this.props.emptyText));
    }
    renderNewCustomTagText() {
        ...
        return (React.createElement("div", { className: `${this.props.rootClassName}__new-custom-tag-text` },
            "Create option \"",
            text,
            "\""));
    }
    renderLoadingText() {
        ...
        return (React.createElement("div", { className: `${this.props.rootClassName}__loading-text` }, this.props.loadingText));
    }
    renderVirtualList() {
        ...
        if (this.state.constrainedHeight) {
            ...
            const constrainedHeight = this.props.maxHeight
                ? Math.min(this.props.maxHeight, this.state.constrainedHeight - constrainedOffset)
        ...
        if (result === undefined) {
            ...
            result = React.createElement(VirtualList, { ...virtualListProps });
        }
    ...
    renderRow({ index }) {
        ...
        const { data, getIdProperty, getDisplayProperty, rootClassName, itemEllipsis, value, activeItem, renderItem, rtl, } = this.props;
    ...
    renderGroup(group, index) {
        ...
        if (result === undefined) {
            result = React.createElement("div", { ...groupProps });
        }
        return result;
    }
    handleOnClick(event) {
    ...
    updateListPosition() {
        ...
        if (positionConfig) {
            const { constrainedHeight, positionRegion, succesfullPosition, position, } = positionConfig;
            this.setState({
                positionRegion,
                succesfullPosition,
